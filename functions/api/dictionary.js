...
const processDictionaryJob = async (jobId, text) => {
    ...
    try {
        ...
        const wordExtractionPrompt = `
            너는 한국어 어휘 분석 전문가야. 주어진 텍스트에서 다음 두 가지 기준에 맞는 단어를 합쳐 최대 20개까지 추출해줘.

            1. 일반적으로 의미가 어렵거나 중요한 핵심 단어, 관용구
            2. 사용자의 관심 주제와 관련된 전문 용어

            각 단어에 대해 태그를 반드시 부여해야 해.
            - 단어가 사용자의 관심 주제에 해당하면, 그 주제를 태그로 사용해줘.
            - 특정 주제와 관련 없는 일반적인 어려운 단어라면 '일반' 태그를 사용해줘.

            - 사용자의 관심 주제: [${knownTopics.join(', ')}]
            - 사용 가능한 태그: [${availableTags.join(', ')}]

            결과는 반드시 다음 형식의 JSON 객체로 반환해줘:
            {"words": [{"word": "추출단어1", "tag": "태그1"}, {"word": "추출단어2", "tag": "태그2"}]}

            ## 예시
            - 사용자 관심 주제: ["IT", "경제"]
            - 텍스트: "온톨로지는 인공지능 분야에서 지식 표현에 사용되는 핵심 개념이며, 이 기술의 시장 가치는 빠르게 상승하고 있다. 다만 인공지능에서의 윤리는 방 안의 코끼리와도 같아 쉽게 간과되곤 한다."
            - 결과: {"words": [{"word": "온톨로지", "tag": "IT"}, {"word": "시장 가치", "tag": "경제"}, {"word": "방 안의 코끼리", "tag": "일반"}]}
